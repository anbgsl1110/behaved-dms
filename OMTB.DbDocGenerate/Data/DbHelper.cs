...
namespace DbDocGenerate.Data
{
    ...
    public partial class DbHelper
    {
        ...
        {
            if (dtEnd.Subtract(dtStart).TotalMilliseconds > 200)
            {
                LogManager.WriteLog(LogFile.Data, "耗时：" + dtEnd.Subtract(dtStart).TotalMilliseconds + "\t" + commandText);
            }

            if (!BaseConfigs.GetDebug || QueryCount >100)
            {
                m_querycount = 0;
                m_querydetail = "";
                return "";
            ...
            if (cmdParams != null && cmdParams.Length > 0)
            {
                foreach(DbParameter param in cmdParams)
                {
                    ...
                    values += "<td>" + Convert.ToString(param.Value) + "</td>";
                }
                ...
                if (m_provider == null)
                {
                    lock (lockHelper)
                    {
                        if (m_provider == null)
                        {
                            try
                            {
                                m_provider = (IDbProvider)Activator.CreateInstance(Type.GetType(string.Format("Study.Data.{0}Provider, Study.Framework", BaseConfigs.GetDbType), false, true));
                            }
                            catch(Exception e)
                            {
                                throw new Exception("请检查HW.config中Dbtype节点数据库类型是否正确，例如：SqlServer、Access、MySql" + e.Message);
                            }
                        }
                    }
                }
        ...
        public static int ExecuteNonQuery(out int id, DbConnection connection, CommandType commandType, string commandText, params DbParameter[] commandParameters)
        {
            ...
            try
            {
                ...
                LogManager.WriteLog(LogFile.Error, commandText + "\n\t" + e.Message);
        ...
        {
            object ec = ExecuteScalar(CommandType.Text, commandText);
            if (ec == null)
            {
                return "";
            }
        ...
        public static int ExecuteDelete(string table, string where)
        {
        ...
        public static bool ChangeField(string table, string filed, int id)
        {
            string sql = "update " + table + " set " + filed + "= ~" + filed + " where id=" + id;
            return ExecuteNonQuery(sql) == 0 ? false : true;
        }
    }
}